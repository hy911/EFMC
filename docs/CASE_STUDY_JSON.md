...
生成 `preview.html`，浏览器打开。章节顺序与编号、每节用了哪种块和哪些选项、文案长短、图片位置、中英对照（右上角切换，没写中文的地方标红底）一目了然，另外会用黄底列出「能过校验但读起来别扭」的建议——连着三节同一版式、metrics 里放的不是数字、数字没交代口径、缺 `location` 之类。

**它是线框图，不是最终效果。** 官网的案例页有完整排版、配色和动效，这里只画结构；真实渲染效果以维护方给的草稿预览链接为准。改一轮重跑一次，不用等对方导入。
...
> `node case-preview.mjs case.json` 生成 `preview.html`，**自己打开看一遍**：章节顺序对不对、
> 哪节太长太短、图片位置合不合适、页面上黄底列的建议要不要改。改完重跑这两条命令。
